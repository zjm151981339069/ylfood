...
<body>
		...
		<div id="content" class="sp-content" style=" padding-bottom: 50px;width:99%">
			<div  class="sp-page-title" >
				<span class="sp-icon"><img src="<%=path %>/img/oms/home.png"></span>
					...
					<span class="sp-color-gray">&gt;</span>
					<span class="sp-color-green">发货订单列表</span>
			</div>
			<div class="sp-content-layout" style="height: 100%">
					...
					<form id="searchConditionForm" method="post">
						...
						<input type="hidden" id="flagSearch" name="flagSearch" value="${searchCriteria.flagSearch}"/>
						 <div ng-controller="ng-controller-datetime" ng-cloak style="height: 100%;float:right;padding-left:10px;">
									<span><label>下单时间:</label><input name="startDate" class="sp-input sp-input-md" placeholder="开始时间" ng-model="startDate" datetimepicker/>-<input name="endDate" class="sp-input sp-input-md" placeholder="截止时间" ng-model="endDate" datetimepicker/></span>
									<span>
										<label>订单状态:</label>
										<select  name="status" style="height:28px" id="orderStatus" value="${searchCriteria.status}"></select>
									</span>
									<span>
										<label>订单类型:</label>
										<select name="type"  style="height:28px" id="orderType" value="${searchCriteria.type}"></select>
									</span>
									<span>
										<label>收货人:</label>
  ...
  <div id='popUpLg' class='sp-popup sp-popup-lg' style="min-width: 350px;min-height:160px;">
    ...
    <div id="pop-logo" class="sp-pop-logo"><span>订单号：<span id="msgOrder"></span></span></div>
...
<script src="js/deliver/list.js" type="text/javascript" ></script>
</html>