...
@Service
@Transactional
public class OperateOrderSiteManagerImpl implements OperateOrderSiteManager{
	@Autowired
    ...
    public void operateOrderBySingleThread(List<String> orderTempCodes) {
        ...
        try {
            ...
            do {
                ...
                /**
                 * 遍历每个订单中的商品，并放到对应的温区,自提点中
                 */
                workOrderExtractManager.getSiteTempAreaCommMap(map, orders);
                count++;
    ...
    public void createDeliveSortRequWorkOrder(Map<String, List<HouseOrderCommodity>> map,Map<HouseOrder, Map<String, List<HouseOrderCommodity>>> SortMapByOrder,Map<String, Integer> commdiCountMap) throws Exception {
        ...
        //温区自提点编码Set集合
        Set<String> areaSiteCodeKeySet = map.keySet();
        //订单Set集合
        Set<HouseOrder> houseOrderKeySet = SortMapByOrder.keySet();
        //自提点和仓库编码Map
        Map<String,String> siteWareHouse =  new HashMap<String, String>();
        //时间格式
        ...
        //保存所有商品到订单商品列表
        for (String areaSite : areaSiteCodeKeySet) {
            List<HouseOrderCommodity> list = map.get(areaSite);
            for (HouseOrderCommodity houseOrderCommodity : list) {
                areaAreaNameMap.put(houseOrderCommodity.getAreaCode(), houseOrderCommodity.getAreaName());
        ...
        for (String areaSite : areaSiteCodeKeySet) {
            ...
            List<HouseOrderCommodity> list = map.get(areaSite);
            //当前温区自提点包含的订单
            List<HouseOrder> currentAreaSiteOrders = new ArrayList<HouseOrder>();
            //出库物品列表
            ...
            //当前温区自提点的总任务数量
            Integer currentAreaSiteTaskNum=0;
            //当前温区编码
            String areaCode = SiteCodeAreaCodeLinkerUtils.getAreaCode(areaSite);
            //当前自提点编码
            String siteCode = SiteCodeAreaCodeLinkerUtils.getSiteCode(areaSite);
            //添加自提点编码到仓库编码
            String wareHouse = siteWareHouse.get(siteCode);
            if(wareHouse==null||"".equals(wareHouse)){
            	ServiceSite queryOneByCode = serviceSiteService.queryOneByCode(siteCode);
            	if(queryOneByCode!=null){
                	wareHouse=queryOneByCode.getDepotCode();
                	siteWareHouse.put(siteCode, wareHouse);
            	}
            }
            //获取当前温区自提点包含的订单
            for (HouseOrder order : houseOrderKeySet) {
                ...
                if(tempset.contains(areaCode)&&order.getSiteCode()!=null&&order.getSiteCode().equals(siteCode)){
                    currentAreaSiteOrders.add(order);
                }
            }
            //出库工单编号
            ...
            outTask.setArea(areaCode);
            outTask.setAreaName(areaAreaNameMap.get(areaCode));
            outTask.setOutCode(deliveCode);
            outTask.setTaskNum(currentAreaSiteTaskNum);
            outTask.setGoodsList(goodsList);
            outTask.setRecCode(requisitionCode);
            outTask.setTaskTime(workOrderTaskTime);
            //仓库编码
            outTask.setRepCode(wareHouse);
            //添加到出库工单列表中
            outTaskList.add(outTask);
            //将出库任务单编号保存到出库任务工单编号Map中
            deliveCodeMap.put(areaSite, deliveCode);
            /**
             * 生成领料任务单
             */
            ReceiveTask receiveTask = new ReceiveTask();
            //添加领料工单数据
            receiveTask.setArea(areaCode);
            receiveTask.setTaskNum(currentAreaSiteTaskNum);
            receiveTask.setRecCode(requisitionCode);
            receiveTask.setGoodsList(receiveGoodsList);
            receiveTask.setAreaName(areaAreaNameMap.get(areaCode));
            receiveTask.setTaskTime(workOrderTaskTime);
            receiveTaskList.add(receiveTask);
            //将领料工单编号存入领料工单编号Map中
            requisitionCodeMap.put(areaSite, requisitionCode);                       
        }
        ...
        for (HouseOrder HouseOrder : houseOrderKeySet) {
        ...
        //依次生成自提点、温区对应的分拣工单列表
        for (String siteCode : extractaddrList) {
            for (String areaCode : areaAreaNameMap.keySet()) {
                logger.info("自提点         "+siteCode+"温区         "+areaCode);
                SortingTask sortingTask = new SortingTask();
                sortingTask.setArea(areaCode);
                ...
                String siteAreaLink = SiteCodeAreaCodeLinkerUtils.siteAreaLink(siteCode, areaCode);
                sortingTask.setRecCode(requisitionCodeMap.get(siteAreaLink));
                sortingTask.setTaskNum(0);
                sortingTask.setAreaName(areaAreaNameMap.get(areaCode));
                sortingTask.setSiteCode(siteCode);
                ...
                Map<String, SortingTask> areaMap = sortTaskMap.get(siteCode);
                //如果自提点中没有对应的map
                if(areaMap==null){
                    Map<String, SortingTask> tempMap= new HashMap<String, SortingTask>();
                    tempMap.put(areaCode, sortingTask);
                    sortTaskMap.put(siteCode, tempMap);
                }else{
                    areaMap.put(areaCode, sortingTask);
                    sortTaskMap.put(siteCode, areaMap);
                }
            }
        }
                ...
                while (iterator.hasNext()) {
                    ...
                    if(tBoolean){
                        ...
                        for (HouseOrderCommodity orderCommodity : list) {
                            ...
                            sortingTaskCommodity.setTypeName(orderCommodity.getTypeName());
                            ...
                            String siteAreaLink = SiteCodeAreaCodeLinkerUtils.siteAreaLink(sortingTask.getSiteCode(),sortingTask.getArea());
                            houseOrderCommodity.setOutCode(deliveCodeMap.get(siteAreaLink));
        ...
        if(createBoolean){
    		...
    		List<HouseOrder> queryHouseOrderList = houseOrderManager.queryHouseOrderListByOrderNos(houseOrderList);
    		for (HouseOrder houseOrder : queryHouseOrderList) {
    			houseOrder.setStatus(HouseOrderStatus.HAS_SORTING.getValue());
    			houseOrder.setTaskTime(workOrderTaskTime);
    		}
    		houseOrderManager.updateHouseOrderStatus(queryHouseOrderList);
    		houseOrderManager.deleteOrderTempByOrderNo(houseOrderList);
